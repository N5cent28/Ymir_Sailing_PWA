...
</p> </div> </section> <!-- Find Members Content --> <section class="py-16 bg-white"> <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"> <!-- Search Section --> <div class="bg-sail-white rounded-lg p-6 shadow-lg mb-8"> <h3 class="text-xl font-bold text-ocean-blue mb-6">Search Members</h3> <div class="flex flex-col md:flex-row gap-4"> <div class="flex-1"> <input type="text" id="memberSearch" placeholder="Search by name (first or last name works)..." class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-ocean-blue"> </div> <button id="clearSearch" class="px-6 py-3 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors font-semibold">
Clear Search
...
Admin
</span>`} </h4> </div> </div> <!-- Bio --> <div class="mb-4"> <p class="text-sm text-gray-700"> ${member.bio || "No bio yet"} </p> </div> <!-- Stats --> <div class="grid grid-cols-2 gap-4 mb-4 text-center"> <div class="bg-blue-50 rounded-lg p-3"> <div class="text-lg font-bold text-blue-600"${addAttribute(`trips-${member.member_number}`, "id")}>-</div> <div class="text-xs text-blue-800">Trips</div> </div> <div class="bg-green-50 rounded-lg p-3"> <div class="text-lg font-bold text-green-600"${addAttribute(`hours-${member.member_number}`, "id")}>-</div> <div class="text-xs text-green-800">Hours</div> </div> </div> <!-- Message Button --> <button${addAttribute(`openMessageTo('${member.member_number}', '${member.name}')`, "onclick")} class="w-full px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-semibold flex items-center justify-center"> <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path> </svg>
Message
</button> </div> </div>`)} </div> <div id="noResults" class="hidden text-center py-12"> <div class="text-6xl mb-4">🔍</div> <h3 class="text-xl font-semibold text-gray-900 mb-2">No members found</h3> <p class="text-gray-600">Try adjusting your search terms or browse all members.</p> </div> </div> </div> </section> </main> ${renderScript($$result2, "/Users/noahnicol/Desktop/ymir-sailing-club/src/pages/en/find-members.astro?astro&type=script&index=0&lang.ts")} ` })}`;
}, "/Users/noahnicol/Desktop/ymir-sailing-club/src/pages/en/find-members.astro", void 0);

const $$file = "/Users/noahnicol/Desktop/ymir-sailing-club/src/pages/en/find-members.astro";
const $$url = "/en/find-members";
